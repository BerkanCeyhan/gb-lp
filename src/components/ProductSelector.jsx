...
import { ShoppingBag, Check, Gift, Plus, X, Heart, Truck, ShieldCheck, Info } from 'lucide-react';
...
export default function ProductSelector() {
  ...
  return (
    <section id="checkout" ref={containerRef} className="py-24 md:py-32 px-6 bg-white relative">
        ...
        <div className="w-full lg:w-1/2 prod-elem">
          ...
          {selectedPkg.compareAtPrice && (
            <div className="flex items-center gap-2 mb-4">
              <span className="inline-flex items-center gap-1.5 bg-[#E8F7F0] text-[#1B5E20] font-sans font-bold text-sm px-3 py-1 rounded-full">
                <Check className="w-3.5 h-3.5" />
                Du sparst {(parseFloat(selectedPkg.compareAtPrice) - parseFloat(selectedPkg.price)).toFixed(2).replace('.', ',')}€
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[10px] font-sans font-semibold uppercase tracking-widest text-dusk-blue bg-light-cyan/60 border border-powder-blue/40 rounded-full px-2.5 py-1 mb-8 hover:bg-light-cyan hover:border-dusk-blue hover:text-jet-black hover:shadow-sm active:scale-95 transition-all cursor-pointer"
            aria-label="Nährwerte anzeigen"
          >
          ...
          {/* URGENCY BANNER */}
          <div className="flex items-center gap-3 mb-6 px-4 py-3 rounded-xl bg-burnt-peach/8 border border-burnt-peach/25">
            <span className="text-burnt-peach text-lg leading-none shrink-0">⏰</span>
            <div className="flex-1 min-w-0">
              <span className="font-sans font-semibold text-[12px] text-jet-black">Neukunden-Aktionspreis — endet heute um Mitternacht</span>
            </div>
            <div className="flex items-center gap-1 shrink-0 font-condensed font-extrabold text-burnt-peach text-sm tabular-nums">
              <span className="bg-burnt-peach/10 rounded px-1.5 py-0.5">{String(timeLeft.h).padStart(2, '0')}</span>
              <span className="opacity-60">:</span>
              <span className="bg-burnt-peach/10 rounded px-1.5 py-0.5">{String(timeLeft.m).padStart(2, '0')}</span>
              <span className="opacity-60">:</span>
              <span className="bg-burnt-peach/10 rounded px-1.5 py-0.5">{String(timeLeft.s).padStart(2, '0')}</span>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {productData.packages.map((pkg) => {
                ...
                return (
                  ...
                  >
                    {pkg.id === 'bundle-6' && (
                      <div className={`absolute top-0 right-0 text-white text-[10px] font-sans font-bold uppercase tracking-widest px-2 py-1 rounded-bl-lg transition-colors duration-300 ${isSelected ? 'bg-burnt-peach' : 'bg-powder-blue'}`}>
                        Beliebteste Wahl
                      </div>
                    )}
                    ...
                    <div className="flex flex-col items-end ml-4 z-10 shrink-0">
                      {pkg.compareAtPrice && (
                        <span className="font-sans text-[11px] text-powder-blue line-through leading-none mb-0.5">
                          {pkg.compareAtPrice}€
                        </span>
                      )}
                      <span className="font-sans font-bold text-jet-black whitespace-nowrap">
                        {pkg.price}€
                      </span>
                      {pkg.compareAtPrice && (
                        <span className="font-sans text-[10px] font-semibold text-[#1B5E20] leading-none mt-0.5">
                          −{(parseFloat(pkg.compareAtPrice) - parseFloat(pkg.price)).toFixed(2).replace('.', ',')}€
                        </span>
                      )}
                    </div>
                  </button>
      ...
      {showNutrition && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-jet-black/60 backdrop-blur-sm"
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-powder-blue/30">
              <div>
                <h3 className="font-condensed font-extrabold uppercase tracking-tight text-xl text-jet-black leading-none">Nährwertangaben</h3>
                <p className="text-[11px] font-sans text-dusk-blue mt-1">Beispiel: Bananensplit – Werte gelten für alle Sorten ähnlich</p>
              </div>
              <button
                onClick={() => setShowNutrition(false)}
                className="w-8 h-8 rounded-full border border-powder-blue flex items-center justify-center hover:bg-light-cyan transition-colors shrink-0"
                aria-label="Schließen"
              >
                <X className="w-4 h-4 text-dusk-blue" />
              </button>
            </div>

            <div className="p-5">
              <div className="flex gap-4 mb-4 text-[11px] font-sans font-semibold text-dusk-blue uppercase tracking-widest">
                <span>Portionsgröße: 3 g</span>
                <span className="text-powder-blue">·</span>
                <span>66 Portionen / Dose</span>
              </div>

              <table className="w-full text-sm font-sans border-collapse">
                <thead>
                  <tr className="border-b-2 border-jet-black">
                    <th className="text-left py-2 font-bold text-jet-black text-[11px] uppercase tracking-wide">Nährwerte</th>
                    <th className="text-right py-2 font-bold text-jet-black text-[11px] uppercase tracking-wide">100 g</th>
                    <th className="text-right py-2 font-bold text-jet-black text-[11px] uppercase tracking-wide">3 g</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ['Energie', '607 kJ / 150 kcal', '18 kJ / 4,5 kcal'],
                    ['Fett', '0 g', '0 g'],
                    ['  davon gesättigte Fettsäuren', '0 g', '0 g'],
                    ['Kohlenhydrate', '7 g', '0,2 g'],
                    ['  davon Zucker', '4 g', '0,1 g'],
                    ['Ballaststoffe', '61 g', '1,83 g'],
                    ['Protein', '0 g', '0 g'],
                    ['Salz', '1,1 g', '0,03 g'],
                    ['Sucralose', '<0,2 g', '<0,01 g'],
                    ['Inulin (von Chicorée)', '71 g', '2,1 g'],
                  ].map(([label, per100, per3], i) => {
                    const isSubRow = label.startsWith('  ');
                    return (
                      <tr key={i} className={`border-b border-powder-blue/20 ${i % 2 === 0 ? 'bg-light-cyan/20' : ''}`}>
                        <td className={`py-2 ${isSubRow ? 'pl-4 text-dusk-blue text-[12px]' : 'font-semibold text-jet-black text-[13px]'}`}>
                          {label.trim()}
                        </td>
                        <td className="text-right py-2 text-[12px] text-dusk-blue">{per100}</td>
                        <td className="text-right py-2 text-[12px] font-semibold text-jet-black">{per3}</td>
                      </tr>
                    );
                  })}
                </tbody>
              ...
              <p className="text-[10px] font-sans text-powder-blue mt-4 leading-relaxed">
                * Zutaten (Bananensplit): Inulin, zuckerfreie dunkle Schokoladenstückchen (12 %), Süßungsmittel: Erythrit, Isomaltulose, Sucralose, Aromen, Emulgator. Zutaten können je nach Sorte leicht variieren.
              </p>
            </div>
          </div>
        </div>
